...
- [x] Build and test
- [x] Create changeset (minor - new feature available)
- [x] Create RichText utility for auto-detecting facets from text:
  - Export/re-export the `RichText` class from `@atproto/api`
  - Create helper function `createFacetsFromText(text, agent?)` that returns `{ text, facets }`
  - Uses RichText.detectFacets() to auto-detect links, hashtags, and mentions
  - Add tests for the utility function (11 tests in rich-text.test.ts)
  - Add documentation and usage examples

**Implementation**: Created `src/lib/rich-text.ts` with:

- `createFacetsFromText(text, agent?)` - async with mention resolution
- `createFacetsFromTextSync(text)` - sync without resolution
- Re-exports `RichText`, `RichTextResult`, `AppBskyRichtextFacet`
...
- [x] Typecheck passes (`pnpm typecheck`)
- [x] Build passes (`pnpm build`)
- [x] Tests pass (`pnpm test` - 102 tests in HypercertOperationsImpl + 11 in rich-text)
- [x] Types export correctly
...
- [x] Add JSDoc documentation to `createCollection()` and `updateCollection()` methods about avatar
- [x] Add usage examples in method documentation showing avatar in collections and projects
- [x] Add/update tests for collections with avatar and banner
- [x] Build and test
- [x] Create changeset (minor - new feature available)

**Note**: JSDoc documentation complete for `createCollection()`, `updateCollection()`, `createProject()`, and
